...
public class stermessage {
	...
	void sethashusage(boolean state) {
		this.forceHashing = state;
	}
	
	void setCryptography(String cryptkey, int cryptcount) {
	...
	void setFromSerial(String serial) {
		...
		if (parts.length < 5) {
			...
			if (forceHashing == true) {
				if (tocontrol.hashCode() != Integer.parseInt(parts[5])) {
					sterlogger.getLogger().info("Hash not matching!"+serial.hashCode()+" "+parts[5]);
				} else {
					sterlogger.getLogger().info("Hash matching!"+serial.hashCode()+" "+parts[5]);
				}
			}
	...
	String getAsSerial() {
		String res = time+" "+from+" "+to+" "+sequence+" "+command;
		if (forceHashing == true) {
